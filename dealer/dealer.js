...
function setup() {
  provider =
    RPC_URL.startsWith("wss") || RPC_URL.startsWith("ws")
...
function log(tid, ...args) {
  console.log(`[T${tid}]`, ...args);
}
...
async function insertPayoutRecords(challengeId, winnerAgentId, tableId) {
  ...
  if (bets && bets.length > 0) {
    ...
    for (const bet of bets) {
      const share =
        totalBetOnWinner > 0n ? (betWinPool * BigInt(bet.amount_dot)) / totalBetOnWinner : 0n;
      if (share > 0n) {
        payouts.push({
          challenge_id: challengeId,
...
async function startTable(tableId, challengeId, agents) {
  ...
  try {
    ...
    console.error(`T${tableId} getTableInfo failed:`, e.message);
    return;
  }
  ...
  log(
    tableId,
    `Starting — ${agentCount} agents, ${sessionLength} hands (challenge: ${challengeId})`
  );
...
async function dealHand(tableId) {
  ...
  if (game.activeSeats.size < 2) {
    ...
    try {
      await sendTx(contract.endSession.bind(contract), tableId);
    } catch (_) {}
  ...
  for (const seat of game.activeSeats) {
    ...
    log(
      tableId,
      `Seat ${seat} (${game.agents[seat]?.agent_name}) hole cards: ${game.holeCards[seat].join(", ")}`
    );
...
function startServer() {
  ...
  app.get("/table/:id", async (req, res) => {
    ...
    try {
      ...
      res.status(404).json({ error: "Table not found or RPC error" });
    }
  });
...
(async function main() {
  ...
  console.log(
    `✅ Dealer live. Polling Supabase every ${POLL_INTERVAL_MS / 1000}s for new poker tables.`
  );
})();
