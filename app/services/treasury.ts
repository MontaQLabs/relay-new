...
function multiplyDotByFraction(amount: string, numerator: number, denominator: number): string {
  ...
  return ((BigInt(amount) * BigInt(Math.round(numerator * 1_000_000))) / BigInt(Math.round(denominator * 1_000_000))).toString();
}
...
export class SupabaseTreasuryService implements ITreasuryService {
  ...
  async calculatePayouts(challengeId: string): Promise<PayoutPlan> {
    ...
    const entryPrizeAmount = multiplyDotByFraction(entryPool, TREASURY_CONFIG.ENTRY_WINNER_SHARE, 1);
    const platformEntryAmount = multiplyDotByFraction(entryPool, TREASURY_CONFIG.ENTRY_PLATFORM_SHARE, 1);
    ...
    if (BigInt(totalWinningBets) > BigInt(0)) {
      for (const bet of winningBets || []) {
        ...
        const payout = (
          (BigInt(betWinnerPool) * BigInt(bet.amount_dot)) / BigInt(totalWinningBets)
        ).toString();
  ...
  async executePayouts(challengeId: string, plan: PayoutPlan): Promise<PayoutResult[]> {
    ...
    results.push(await recordPayout(
      plan.entryPrize.wallet,
      plan.entryPrize.amount,
      'entry_prize'
    ));
    ...
    results.push(await recordPayout(
      plan.platformBetFee.wallet,
      plan.platformBetFee.amount,
      'platform_bet_fee'
    ));
