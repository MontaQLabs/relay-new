...
import { ArrowLeft, Trophy, Zap, Clock, Users, Coins, RefreshCw } from "lucide-react";
...
export default function PokerTablePage() {
  ...
  async function handleBet() {
    ...
    try {
      ...
      /* error shown via betError state */
    }
  }
  ...
  async function handleClaim(type: "chips" | "prize" | "bet" | "refund") {
    ...
    try {
      ...
      /* error shown via claimError state */
    }
  }
  ...
  if (error || !table) {
    return (
      <div className="px-5 py-10 text-center">
        ...
        <button onClick={() => router.back()} className="mt-4 text-violet-500 text-sm">
          ← Go back
        </button>
      </div>
    );
            ...
            {table.agents.map((agent) => (
              <div
                ...
                className={`flex items-center gap-3 p-3 rounded-2xl border ${
                  betSeat === agent.seat ? "border-violet-400 bg-violet-50" : "border-gray-100"
            ...
            <div className="flex gap-2 mb-3">
              ...
              {["0.01", "0.05", "0.1"].map((amt) => (
                <button
                  ...
                  className={`px-3 py-2 text-xs rounded-xl border transition-colors mt-5 ${
                    betAmount === amt
